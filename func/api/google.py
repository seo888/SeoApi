...
class Google:
    ...
    async def request_get(self, url, headers=None, params=None, use_ip="127.0.0.1"):
        ...
        async with httpx.AsyncClient(
            headers=headers, params=params, http2=True, transport=transport
        ) as client:
    ...
    async def search(self, querry, num):
        ...
        resp = await self.request_get(
            url, headers=headers, params=params, use_ip=use_ip
        )
        return resp.text
    ...
    async def get_data(self, querry, num):
        ...
        try:
            ...
            # 相关搜索 关键词
            related = tree.xpath('//div[@id="bres"]//a//b/text()')
            # related = (
            #     tree.xpath('//div[@class="s75CSd OhScic AB4Wff"]//b/text()')
            #     if related == []
            #     else related
            # )
            # related = (
            #     tree.xpath('//div[@class="wQiwMc related-question-pair"]//span/text()')
            #     if related == []
            #     else related
            # )
            # 其他人搜
            ...
            more = (
                tree.xpath('//span[@class="JCzEY ZwRhJd"]/span/text()')
                if more == []
                else more
            )
            return {
    ...
    async def get_pulldown(self, querry):
        ...
        try:
            ...
            # url = f"https://www.google.com/complete/search?q={querry}&cp=0&client=desktop-gws-wiz-on-focus-serp&xssi=t&gs_pcrt=3&hl=zh-CN&authuser=0&pq={querry}&psi=CU-WZcPUMcaRseMPg5miyA0.1704349911180&dpr=1.5&ofp=EAEy7wEKFAoS572R6LWM5bmz5Y-w5aSn5YWoChcKFemdoOiwseeahOe9kei1jOW5s-WPsAoUChLmraPop4TotYzljZrlubPlj7AKHQob5YWo55CD5pyA5aSn55qE5Y2a5b2p5bmz5Y-wChEKD-e9kei1jOeKr-azleWQlwoaChjotYzljZrlubPlj7DpqazmnaXopb_kupoKEAoO572RIOi1jCDnvZHnq5kKFAoS5Zu95aSW6LWM5Y2a5bmz5Y-wCh0KG-WkqumYs-Wfjue9kee7nOi1jOWNmuW5s-WPsAoRCg_nur_kuIrotYzljZphcHAQRw"
            use_ip = await self.func.use_ip("google")
            transport = httpx.AsyncHTTPTransport(local_address=use_ip)
            ...
            return {
                "keyword": querry,
                "pull_down_words": pull_down_words,
                "success": True,
            }
        except Exception as err:
            print(err)
