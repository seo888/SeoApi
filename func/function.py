...
class Func():
    ...
    def geminiToken_useip(self):
        ...
        # 非阻塞模式
        try:
            token_ip = self.queue.get_nowait()  # 这里会立即抛出异常，因为队列为空
        except Empty:
            print('进来了')
            ips = sorted(list(set(self.ips)))
            tokens = self.get_lines("config/gemini_tokens.txt")
            for index, token in enumerate(tokens[:len(ips)]):
                self.queue.put(token + "|" + ips[index])
            print('写入了')
            token_ip = self.queue.get()
        print(token_ip)
        return token_ip
